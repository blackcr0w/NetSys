function Y = schedule3_launcher2(portion)
...
s1 = zeros(1, n * 3); % si is the access of app i, init to all-zero;
% this is the case when app1 is scheduled twice than app2
s2 = zeros(1, n);
rand_temp = [-1 -1 -1 -1]; % rand_temp is the memory access of app1 and app2 in this loop
...
for i = 1 : n
    ...
    s1(3 * i - 2) = rand_temp(1);
    s1(3 * i - 1) = rand_temp(2);
    s1(3 * i) = rand_temp(3);
    s2(i) = rand_temp(4);
    replace_2(s1(3 * i - 2));
    replace_2(s1(3 * i - 1));
