...
N = 50  # run the program for N times
S1 = []  # S1 represents the memory access series of app1
S2 = []  # S2 represents the memory access series of app2
LRU = [-1 for _ in range(3)] # LRU keeps track of the LRU of all cachelines
LRU_CURRENT = 0  # the current time
CL = [-1 for _ in range(3)]  # init the value  to -1
LRU_MAX = float('inf')


def raplace(cl_to_update, rand_temp):
